...
try {
  ...
  for (const name of ['016_player_acquisition.sql', '017_player_retention.sql', 'verify_player_retention.sql', '018_gameplay_attempts.sql', '019_gameplay_retry_duration.sql', '020_gameplay_placements.sql']) {
    await db.exec(await readFile(resolve(mobileRoot, 'supabase/manual', name), 'utf8'));
    console.log('PASS mobile dependency', name);
  }
  for (const name of ['acquisition-reporting.sql', 'acquisition-reporting.verify.sql', 'retention-reporting.sql', 'retention-reporting.sql', 'retention-reporting.verify.sql', 'engagement-reporting.sql', 'engagement-reporting.sql', 'engagement-reporting.verify.sql']) {
  ...
  for (const role of ['anon', 'authenticated']) {
    for (const query of ['select public.float_retention_report(current_date,current_date,current_date,current_date)', 'select public.float_engagement_report(current_date,current_date)']) {
  ...
  await db.query('select public.float_engagement_report(current_date,current_date)');
