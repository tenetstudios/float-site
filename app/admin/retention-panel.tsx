...
function Cohorts({ title, labels, rows, heatmap = false }: { title: string; labels: string[]; rows: RetentionGroup[]; heatmap?: boolean }) {
  return <section className={styles.panel}><h2>{title}</h2><p className={styles.muted}>{heatmap ? "Each row is an acquisition first-seen UTC cohort. Darker blue means higher exact-day retention." : "Top 50 groups by acquisition installs · selected cohorts"}</p><div className={styles.tableScroll} tabIndex={0} role="region" aria-label={title}><SortableTable labels={[...labels, "Installs / tracked (sort by installs)", "D1 ? retained / eligible (sort by %)", "D7 ? retained / eligible (sort by %)", "D30 ? retained / eligible (sort by %)", "Avg. lifetime active days", "Avg. lifetime sessions"]} rows={rows.map(row => [
    ...row.values.map(value => ({ value })),
    { value: row.metrics.installs, content: <>{count(row.metrics.installs)} / {count(row.metrics.tracked)}</> },
    ...(["d1", "d7", "d30"] as const).map(key => ({ value: row.metrics[key].percent, content: <Rate rate={row.metrics[key]} />, style: heatmap ? { background: row.metrics[key].percent === null ? "#edf0f4" : `rgba(14,142,247,${.08 + row.metrics[key].percent / 100 * .35})` } : undefined })),
    ...[row.metrics.averageActiveDays, row.metrics.averageSessions].map(value => ({ value, content: count(value) })),
  ])} /></div>{!rows.length && <p className={styles.muted}>No matching cohorts.</p>}</section>;
}
export default function RetentionPanel({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return <div data-retention-panel>
    ...
    {report && <div aria-busy={busy}>{error && <p className={styles.stale}>Showing the last successful retention report. These figures may be out of date.</p>}<p className={styles.period}>Selected cohorts: {filters.start} – {filters.end} · as of {report.asOf} UTC</p>
      ...
      <section className={styles.panel}><h2>Sessions per active day</h2><p>Activity window: {filters.activityStart} – {filters.activityEnd} UTC · selected cohorts only</p><p>{count(report.activity.sessionsPerActiveDay)} sessions per installation-active-day · {count(report.activity.sessionStarts)} observed starts / {count(report.activity.activeDays)} observed active days</p><p className={styles.muted}>Continued activity across midnight can add an active day with zero new session starts. No observed rows means an unavailable rate, not zero retention.</p><div className={styles.tableScroll} tabIndex={0} role="region" aria-label="Daily sessions"><SortableTable labels={["UTC activity date", "Session starts", "Installation-active-days", "Sessions / active day"]} rows={report.daily.map(row => [{ value: row.day }, ...[row.sessionStarts, row.activeDays, row.sessionsPerActiveDay].map(value => ({ value, content: count(value) }))])} /></div></section>
      <section className={styles.panel}><h2>Days since last session</h2><p className={styles.muted}>From the last session start’s UTC date, not last_seen_at. Untracked installations are Unavailable.</p><div className={styles.tableScroll}><SortableTable labels={["Days since session start", "Installations"]} rows={report.inactivity.map(row => [{ value: row.label === "Today" ? 0 : /^\d/.test(row.label) ? parseInt(row.label, 10) : null, content: row.label }, { value: row.installs, content: count(row.installs) }])} /></div></section>
