...
function Breakdown({ title, labels, rows }: { title: string; labels: string[]; rows: Group[] }) {
  return <section className={styles.panel}><h2>{title}</h2><p className={styles.muted}>Top {TOP} groups by installs · filters apply</p><div className={styles.tableScroll} tabIndex={0} role="region" aria-label={`${title} table`}><table><thead><tr>{labels.map(label => <th key={label} scope="col">{label}</th>)}<th scope="col">Installs</th></tr></thead><tbody>{rows.map((row, i) => <tr key={i}>{row.values.map((value, j) => <td key={j}>{value === null ? <em>Unknown</em> : value === "" ? <em>Empty value</em> : value}</td>)}<td>{number(row.installs)}</td></tr>)}</tbody></table></div>{!rows.length && <p className={styles.muted}>No matching installations.</p>}</section>;
}
